...
function renderInlineValue(value) {
    ...
    if (Array.isArray(value)) {
        ...
        return Array.isArray(value[0]) ? formatMatrix(value) : formatMatrix([value]);
    }
    return `<span class="font-mono text-sm text-slate-700">${formatCell(value, getUiPrecision())}</span>`;
}

function renderBeforeAfterMatrices(beforeMatrix, afterMatrix, numVars = -1) {
    if (!beforeMatrix && !afterMatrix) return '';

    let html = `<div class="mt-3 flex flex-wrap items-start justify-center gap-4">`;
    if (beforeMatrix) {
        html += `<div class="matrix-display">${formatMatrix(beforeMatrix, 'Truoc', numVars)}</div>`;
    }
    if (afterMatrix) {
        html += `<div class="matrix-display">${formatMatrix(afterMatrix, 'Sau', numVars)}</div>`;
    }
    html += `</div>`;
    return html;
}

function renderRowOperationsDetails(rowOperations) {
    if (!rowOperations || rowOperations.length === 0) {
        return `<p class="mt-2 text-sm text-gray-500">Khong co phep khu nao can thuc hien o buoc nay.</p>`;
    }

    let html = `<details class="mt-3 rounded-lg border border-slate-200 bg-white p-3">
        <summary class="cursor-pointer text-sm font-medium text-slate-700">Chi tiet cac phep bien doi hang</summary>
        <div class="mt-3 space-y-4">`;

    rowOperations.forEach((op, index) => {
        html += `<div class="rounded-md border border-slate-100 bg-slate-50 p-3">
            <p class="font-mono text-sm text-slate-800">${op.operation || `Row operation ${index + 1}`}</p>
            ${op.factor !== undefined ? `<p class="mt-1 text-sm text-slate-600">factor = ${renderInlineValue(op.factor)}</p>` : ''}
            <div class="mt-3 flex flex-wrap items-start justify-center gap-4">
                ${op.row_before ? `<div class="matrix-display">${formatMatrix([op.row_before], `R${(op.target_row ?? 0) + 1} truoc`)}</div>` : ''}
                ${op.pivot_row_snapshot ? `<div class="matrix-display">${formatMatrix([op.pivot_row_snapshot], `R${(op.pivot_row ?? 0) + 1}`)}</div>` : ''}
                ${op.row_after ? `<div class="matrix-display">${formatMatrix([op.row_after], `R${(op.target_row ?? 0) + 1} sau`)}</div>` : ''}
            </div>
        </div>`;
    });
...
function renderTriangularSolveSteps(title, steps, resultLabel) {
    ...
    let html = `<h3 class="text-lg font-semibold text-gray-700 mt-6 mb-2">${title}</h3>`;
    steps.forEach((step, index) => {
        html += `<div class="mb-4 rounded-lg bg-blue-50 p-3 shadow-sm">
            <p class="text-sm font-semibold text-slate-800">Buoc ${index + 1}: hang ${step.row + 1}</p>
            <div class="mt-2 grid grid-cols-1 gap-3 md:grid-cols-2">
                <div class="rounded bg-white p-3 text-sm">
                    <p>diag = ${renderInlineValue(step.diag)}</p>
                    <p class="mt-2">rhs = ${renderInlineValue(step.rhs)}</p>
                    <p class="mt-2">tong da biet = ${renderInlineValue(step.known_sum)}</p>
                    <p class="mt-2">tu so = ${renderInlineValue(step.numerator)}</p>
                    <p class="mt-2">gia tri moi = ${renderInlineValue(step.result)}</p>
                </div>
                <div class="rounded bg-white p-3">
                    ${step.coefficients && step.coefficients.length ? `<div class="matrix-display">${formatMatrix([step.coefficients], 'He so')}</div>` : '<p class="text-sm text-gray-500">Khong co he so da biet.</p>'}
                    ${step.known_values && step.known_values.length ? `<div class="mt-3 matrix-display">${formatMatrix(step.known_values, 'Gia tri da biet')}</div>` : ''}
                </div>
            </div>
            ${step.solution_so_far ? `<div class="mt-3 matrix-display">${formatMatrix(step.solution_so_far, resultLabel)}</div>` : ''}
        </div>`;
    });
    return html;
}
...
function renderGaussBackwardSteps(steps) {
    ...
    let html = `<h3 class="text-lg font-semibold text-gray-700 my-4">Cac buoc the nguoc:</h3>`;
    steps.forEach((step, index) => {
        html += `<div class="mb-4 rounded-lg bg-blue-50 p-3 shadow-sm">
            <p class="text-sm text-gray-800"><b>Buoc ${index + 1}:</b> ${step.message}</p>
            <div class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
                <div class="rounded bg-white p-3 text-sm">
                    <p>pivot = ${renderInlineValue(step.pivot_value)}</p>
                    <p class="mt-2">ve phai = ${renderInlineValue(step.rhs)}</p>
                    <p class="mt-2">tong da biet = ${renderInlineValue(step.known_contribution)}</p>
                    <p class="mt-2">tu so = ${renderInlineValue(step.numerator)}</p>
                    <p class="mt-2">x hien tai = ${renderInlineValue(step.computed_value)}</p>
                </div>
                <div class="rounded bg-white p-3">
                    ${step.coefficients && step.coefficients.length ? `<div class="matrix-display">${formatMatrix([step.coefficients], 'He so')}</div>` : '<p class="text-sm text-gray-500">Khong co he so ben phai pivot.</p>'}
                    ${step.known_solution_values && step.known_solution_values.length ? `<div class="mt-3 matrix-display">${formatMatrix(step.known_solution_values, 'Gia tri da biet')}</div>` : ''}
                </div>
            </div>
            ${step.solution_so_far ? `<div class="mt-3 matrix-display">${formatMatrix(step.solution_so_far, 'X')}</div>` : ''}
        </div>`;
    });
    return html;
}
...
function renderLuStepDetails(step) {
    let html = `<div class="mb-4 p-3 bg-gray-50 rounded-lg shadow-sm">
        <p class="text-sm text-gray-800 mb-2">${step.message}</p>
        ${step.pivot_value !== undefined ? `<p class="text-sm text-slate-600 mb-2">pivot = ${renderInlineValue(step.pivot_value)}</p>` : ''}
        <div class="flex flex-wrap items-center justify-center gap-4">
            <div class="matrix-display">${formatMatrix(step.L, 'L')}</div>
            <div class="matrix-display">${formatMatrix(step.U, 'U')}</div>
        </div>`;

    if (step.u_calculations?.length || step.l_calculations?.length) {
        html += `<details class="mt-3 rounded-lg border border-slate-200 bg-white p-3">
            <summary class="cursor-pointer text-sm font-medium text-slate-700">Chi tiet tinh toan trong buoc nay</summary>
            <div class="mt-3 space-y-3">`;

        step.u_calculations?.forEach(calc => {
            html += `<div class="rounded border border-slate-100 bg-slate-50 p-3 text-sm">
                <p class="font-mono">u<sub>${calc.row + 1},${calc.col + 1}</sub> = a - tong = ${renderInlineValue(calc.result)}</p>
                <p class="mt-1">a = ${renderInlineValue(calc.a_value)}, tong = ${renderInlineValue(calc.sum_value)}</p>
            </div>`;
        });

        step.l_calculations?.forEach(calc => {
            html += `<div class="rounded border border-slate-100 bg-slate-50 p-3 text-sm">
                <p class="font-mono">l<sub>${calc.row + 1},${calc.col + 1}</sub> = (${renderInlineValue(calc.numerator)}) / (${renderInlineValue(calc.denominator)}) = ${renderInlineValue(calc.result)}</p>
                <p class="mt-1">a = ${renderInlineValue(calc.a_value)}, tong = ${renderInlineValue(calc.sum_value)}</p>
            </div>`;
        });
...
function renderCholeskyFactorizationSteps(steps) {
    ...
    let html = `<h3 class="text-lg font-semibold text-gray-700 mt-6 mb-2">Cac buoc phan ra Cholesky:</h3>`;
    steps.forEach((step, index) => {
        html += `<div class="mb-4 rounded-lg bg-amber-50 p-3 shadow-sm">
            <p class="text-sm font-semibold text-slate-800">Buoc ${index + 1}: tinh q<sub>${step.step_index + 1},${step.step_index + 1}</sub></p>
            <p class="mt-2 text-sm">Tong duong cheo = ${renderInlineValue(step.diagonal_sum)}</p>
            <p class="mt-2 text-sm">Bieu thuc duoi can = ${renderInlineValue(step.radicand)}</p>
            <p class="mt-2 text-sm">Gia tri q<sub>${step.step_index + 1},${step.step_index + 1}</sub> = ${renderInlineValue(step.q_kk)}</p>
            ${step.partial_Q ? `<div class="mt-3 matrix-display">${formatMatrix(step.partial_Q, 'Q tam thoi')}</div>` : ''}`;

        if (step.off_diagonal_calculations?.length) {
            html += `<details class="mt-3 rounded-lg border border-amber-200 bg-white p-3">
                <summary class="cursor-pointer text-sm font-medium text-slate-700">Chi tiet cac phan tu q<sub>${step.step_index + 1},j</sub></summary>
                <div class="mt-3 space-y-3">`;
            step.off_diagonal_calculations.forEach(calc => {
                html += `<div class="rounded border border-slate-100 bg-slate-50 p-3 text-sm">
                    <p class="font-mono">q<sub>${calc.row + 1},${calc.col + 1}</sub> = (${renderInlineValue(calc.numerator)}) / (${renderInlineValue(calc.denominator)}) = ${renderInlineValue(calc.result)}</p>
                    <p class="mt-1">cross term = ${renderInlineValue(calc.cross_term)}</p>
                </div>`;
            });
            html += `</div></details>`;
        }
...
function renderCholeskyReconstructionSteps(steps) {
    ...
    let html = `<h3 class="text-lg font-semibold text-gray-700 mt-6 mb-2">Buoc dung lai nghiem:</h3>`;
    steps.forEach((step, index) => {
        html += `<div class="mb-4 rounded-lg bg-emerald-50 p-3 shadow-sm">
            <p class="text-sm text-slate-800"><b>Buoc ${index + 1}:</b> ${step.description}</p>
            <div class="mt-3 flex flex-wrap items-center justify-center gap-4">
                ${step.matrix_at ? `<div class="matrix-display">${formatMatrix(step.matrix_at, 'Aᵀ')}</div>` : ''}
                ${step.auxiliary_solution ? `<div class="matrix-display">${formatMatrix(step.auxiliary_solution, 'Lambda')}</div>` : ''}
                ${step.result ? `<div class="matrix-display">${formatMatrix(step.result, 'X')}</div>` : ''}
            </div>
        </div>`;
    });
    return html;
...
export function renderMatrixSolution(container, data) {
    ...
    if (data.method === "Cholesky" && data.factorization_note) {
        html += `
            <div class="mx-auto mb-6 max-w-4xl rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                ${data.factorization_note}
            </div>`;
    }
    if (data.method === "Cholesky" && data.solution_note) {
        html += `
            <div class="mx-auto mb-6 max-w-4xl rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                ${data.solution_note}
            </div>`;
    }
    if (data.method === "Cholesky" && data.original_factorization) {
        ...
        html += `
            <div class="mx-auto mb-6 max-w-4xl rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-800">
    ...
    if (data.decomposition) {
        if (
            ...
            html += `<h3 class="text-lg font-semibold text-gray-700 mt-6 mb-2">Biến đổi trực tiếp từ A sang Qᵀ và Q (A = QᵀQ):</h3>`;
            html += `<div class="flex flex-wrap items-center justify-center gap-4">`;
            if (data.original_factorization.Qt) html += `<div class="matrix-display">${formatMatrix(data.original_factorization.Qt, 'Qᵀ')}</div>`;
            if (data.original_factorization.Q) html += `<div class="matrix-display">${formatMatrix(data.original_factorization.Q, 'Q')}</div>`;
            html += `</div>`;
        }

        // Ma trận M, d (Cholesky, nếu A không đối xứng)
        if (data.decomposition.M) {
            html += `<h3 class="text-lg font-semibold text-gray-700 mt-6 mb-2">${data.working_system_description || 'Hệ phương trình tương đương:'}</h3>`;
            html += `<div class="flex flex-wrap items-center justify-center gap-4">`;
            html += `<div class="matrix-display">${formatMatrix(data.decomposition.M, 'M')}</div>`;
            html += `<div class="matrix-display">${formatMatrix(data.decomposition.d, data.decomposition_rhs_label || 'd')}</div>`;
            html += `</div>`;
        }
        ...
        // Ma trận Q, Qᵀ (cho Cholesky)
        if (data.method === "Cholesky" && data.decomposition.M && (data.decomposition.Q || data.decomposition.Qt)) {
            html += `<h3 class="text-lg font-semibold text-gray-700 mt-6 mb-2">Phân rã Cholesky trên ma trận làm việc (M = QᵀQ):</h3>`;
            html += `<div class="flex flex-wrap items-center justify-center gap-4">`;
            if (data.decomposition.Qt) html += `<div class="matrix-display">${formatMatrix(data.decomposition.Qt, 'Qᵀ')}</div>`;
            if (data.decomposition.Q) html += `<div class="matrix-display">${formatMatrix(data.decomposition.Q, 'Q')}</div>`;
            html += `</div>`;
        }
    }
    ...
    if (data.intermediate_y) {
         html += `
            <div class="mt-6">
                <h3 class="text-lg font-semibold text-gray-700 mb-2">Vector trung gian (Y), với QᵀY = d:</h3>
    ...
    if (data.auxiliary_solution) {
        html += `
            <div class="mt-6">
                <h3 class="text-lg font-semibold text-gray-700 mb-2">Biến phụ (${data.auxiliary_solution_label || 'Λ'}):</h3>
                <div class="matrix-display">${formatMatrix(data.auxiliary_solution, data.auxiliary_solution_label || 'Λ')}</div>
            </div>`;
    }
    ...
    if (data.solution) {
        html += `
            <div class="my-6">
                <h3 class="text-lg font-semibold text-gray-700 mb-2">${data.solution_heading || 'Nghiệm tìm được (X):'}</h3>
    ...
    if (data.general_solution) {
        html += `
            <div class="my-6">
                <h3 class="text-lg font-semibold text-gray-700 mb-2">${data.general_solution_heading || 'Nghiệm tổng quát:'}</h3>
    ...
    if (data.method === "Cholesky" && data.residual) {
        ...
        html += `
            <div class="mt-6">
                <h3 class="text-lg font-semibold text-gray-700 mb-2">Sai số dư R = AX - B:</h3>
                <div class="matrix-display">${formatMatrix(data.residual, 'R')}</div>
                <p class="mt-2 text-center text-sm text-gray-600">||R||₂ = ${residualNorm}</p>
            </div>`;
    }
...
function renderLatexMatrixTable(matrixLatex) {
    if (!matrixLatex || matrixLatex.length === 0) return '';

    let matrixHtml = `<div class="matrix-display">
        <div class="matrix-container">
            <table class="matrix-table">`;

    matrixLatex.forEach((row) => {
        matrixHtml += '<tr>';
        row.forEach((cell) => {
            matrixHtml += `<td><span class="jacobian-cell">${cell}</span></td>`;
        });
        matrixHtml += '</tr>';
    });
...
function renderLatexNormAnalysis(expressionsLatex, maxima, labelPrefix, indexSymbol, normSuffix, contractionFactor, precision) {
    ...
    expressionsLatex.forEach((expressionLatex, index) => {
        const maxValue = maxima?.[index];
        const hasMaxValue = isFiniteNumber(maxValue);
        html += `
            <div class="p-3 bg-white rounded border border-gray-200">
                <p class="text-sm">
                    <span class="katex-render" data-formula="${labelPrefix}_{${index + 1}}(X) = ${expressionLatex}"></span>
                </p>
                ${hasMaxValue ? `
                    <p class="text-xs text-gray-600 mt-2">
                        <span class="katex-render" data-formula="\\max_{X \\in S(\\alpha, r)} ${labelPrefix}_{${index + 1}}(X) \\approx ${maxValue.toFixed(precision)}"></span>
                    </p>` : ''}
            </div>`;
    });

    if (isFiniteNumber(contractionFactor)) {
        html += `
            <div class="p-3 bg-blue-50 rounded border border-blue-200">
                <p class="text-sm font-semibold text-blue-800">
                    <span class="katex-render" data-formula="K_{${normSuffix}} = \\max_{${indexSymbol}} \\max_{X \\in S(\\alpha, r)} ${labelPrefix}_{${indexSymbol}}(X) \\approx ${contractionFactor.toFixed(precision)}"></span>
                </p>
            </div>`;
    }
...
function resolveContractionFactor(primaryValue, fallbackValue) {
    if (isFiniteNumber(primaryValue)) return primaryValue;
...
export function renderNonlinearSystemSolution(container, data) {
    ...
    if (data.method === 'Phương pháp Lặp đơn' && data.convergence_info) {
        ...
        const kInfinity = resolveContractionFactor(info.contraction_factor_infinity, info.max_row_sum);
        const kOne = resolveContractionFactor(info.contraction_factor_one, info.max_col_sum);
        const selectedK = resolveContractionFactor(
            info.contraction_factor_K,
            info.norm_used_for_K === 'infinity' ? kInfinity : kOne
        );
        const selectedNormExpressions = info.selected_norm_sum_expressions_latex
            ?? (info.norm_used_for_K === 'infinity' ? info.row_sum_expressions_latex : info.col_sum_expressions_latex);
        const selectedNormMaxima = info.selected_norm_sum_maxima
            ?? (info.norm_used_for_K === 'infinity' ? info.row_sum_maxima : info.col_sum_maxima);
        const selectedNormLabelPrefix = info.selected_norm_sum_label_prefix
            ?? (info.norm_used_for_K === 'infinity' ? 'R' : 'C');
        const selectedNormIndexSymbol = info.selected_norm_sum_index_symbol
            ?? (info.norm_used_for_K === 'infinity' ? 'i' : 'j');
        ...
        html += `<div class="my-6 p-4 bg-gray-50 rounded-lg border">
            ...
            ${data.jacobian_matrix_latex ? `
                <div class="mb-4">
                    <h4 class="text-md font-semibold text-gray-600 mb-2">Ma trận Jacobi của ánh xạ lặp <span class="katex-render" data-formula="J_{\\phi}(X)"></span>:</h4>
                    ${renderLatexMatrixTable(data.jacobian_matrix_latex)}
                    <p class="mt-2 text-xs text-gray-600">Từ ma trận này, trước hết cộng trị tuyệt đối theo hàng hoặc cột đúng với chuẩn đang xét, rồi mới lấy GTLN của từng tổng trên hình cầu <span class="katex-render" data-formula="S(\\alpha, r)"></span>.</p>
                </div>` : ''}

            <div class="mb-4">
                <h4 class="text-md font-semibold text-gray-600 mb-2">Các tổng hàng/cột cuối cùng theo chuẩn đã chọn <span class="katex-render" data-formula="${normSymbol}"></span>:</h4>
                ${renderLatexNormAnalysis(
                    selectedNormExpressions,
                    selectedNormMaxima,
                    selectedNormLabelPrefix,
                    selectedNormIndexSymbol,
                    normSymbol,
                    selectedK,
                    precision
                )}
            ...
            <div class="mt-4 text-sm space-y-2">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ${(info.sphere_center && info.sphere_radius !== undefined) ? `<p>Tâm <span class="katex-render" data-formula="\\alpha"></span>: <span class="font-mono">[${info.sphere_center.map((value) => value.toFixed(precision)).join('; ')}]</span></p>
                    <p>Bán kính <span class="katex-render" data-formula="r"></span>: <span class="font-mono">${info.sphere_radius.toFixed(precision)}</span></p>` : ''}
                </div>
                ${isFiniteNumber(kInfinity) && isFiniteNumber(kOne) ? `<p>So sánh hai hệ số co: <span class="katex-render" data-formula="K_{\\infty} \\approx ${kInfinity.toFixed(precision)},\\quad K_{1} \\approx ${kOne.toFixed(precision)}"></span></p>` : ''}
                ${isFiniteNumber(selectedK) ? `<p class="mt-2">Sử dụng chuẩn <span class="katex-render" data-formula="${normSymbol}"></span> vì cho hệ số co nhỏ hơn.</p>
                <p class="text-lg font-bold text-blue-700">Hệ số co được chọn <span class="katex-render" data-formula="K \\approx ${selectedK.toFixed(precision)}"></span></p>` : ''}
            </div>
        </div>`;
    }
    ...
    if (data.jacobian_matrix_latex && data.method !== 'Phương pháp Lặp đơn') {
        html += `<div class="my-6">
...
export function renderFindIntervalsSolution(container, data) {
    ...
    if (data.source_description) {
        html += `<div class="mb-4 p-3 bg-slate-50 border border-slate-200 rounded-lg text-center text-sm text-slate-700">
            ${data.source_description}
            ${Number.isFinite(data.total_valid_nodes) ? `<div class="mt-1 font-medium">Số mốc hợp lệ sau khi làm sạch: ${data.total_valid_nodes}</div>` : ''}
        </div>`;
    }
