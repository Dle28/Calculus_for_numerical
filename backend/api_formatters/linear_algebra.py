...
def _format_complex_value(value):
...
def _format_numeric_data(data):
    if isinstance(data, dict):
        return {key: _format_numeric_data(value) for key, value in data.items()}
    if isinstance(data, np.ndarray):
        return _format_numeric_data(data.tolist())
...
def _legacy_format_cholesky_result_unused(result):
    decomp = result['decomposition']
    formatted = {
        "method": "Cholesky",
        "status": result['status'],
        "message": "Hệ có nghiệm duy nhất tìm bằng phân rã Cholesky.",
        ...
        "decomposition": {
            "Q": _format_numeric_data(decomp['Q']),
            "Qt": _format_numeric_data(decomp['Qt'])
        }
    }
...
def format_cholesky_result(result):
    ...
    formatted = {
        "method": "Cholesky",
        "status": result['status'],
        "message": result.get('message', 'Khong co mo ta cho ket qua Cholesky.'),
        "transformation_message": result['transformation_message'],
        "factorization_note": result.get('factorization_note'),
