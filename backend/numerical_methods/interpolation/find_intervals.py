...
def _build_dataframe_from_manual_input(
    x_nodes: Iterable[float],
    y_nodes: Iterable[float],
) -> pd.DataFrame:
    x_list = list(x_nodes)
    y_list = list(y_nodes)
    ...
    if len(x_list) != len(y_list):
        raise ValueError(
            f"Số lượng mốc x ({len(x_list)}) phải bằng số lượng giá trị y ({len(y_list)})."
        )
...
def find_root_intervals(
    ...
    num_nodes: int,
    method: str = 'both',
    x_nodes: Iterable[float] | None = None,
    y_nodes: Iterable[float] | None = None,
    input_mode: str = 'csv',
) -> Dict[str, Any]:
    """
    Tìm các khoảng cách ly nghiệm f(x) = y_bar từ bảng dữ liệu và mở rộng
    khoảng đó ra k mốc lân cận dựa trên phương pháp mở rộng được chọn.

    Nguồn dữ liệu có thể là:
    - file_stream: dữ liệu đọc từ CSV
    - x_nodes, y_nodes: dữ liệu người dùng nhập tay

    method: 'left' | 'right' | 'both' (mặc định 'both')
    """
    if input_mode == 'manual':
        df = _build_dataframe_from_manual_input(x_nodes or [], y_nodes or [])
        source_description = (
            "Dữ liệu được nhập tay từ hai danh sách xᵢ và yᵢ."
        )
    else:
        if file_stream is None:
            raise ValueError("Không có dữ liệu CSV để xử lý.")
        try:
            df = pd.read_csv(
                file_stream,
                header=None,
        ...
        source_description = (
            "Dữ liệu được đọc từ file CSV với hai cột đầu tiên tương ứng là x và y."
        )
