...
def find_intervals_route():
    try:
        data = request.form
        input_mode = data.get('input_mode', 'csv')
        ...
        if input_mode == 'manual':
            x_nodes_raw = data.get('x_nodes', '')
            y_nodes_raw = data.get('y_nodes', '')

            if not x_nodes_raw.strip() or not y_nodes_raw.strip():
                return jsonify({"error": "Vui lòng nhập đầy đủ danh sách xᵢ và yᵢ."}), 400

            x_nodes = [float(x) for x in x_nodes_raw.replace(',', ' ').split()]
            y_nodes = [float(y) for y in y_nodes_raw.replace(',', ' ').split()]
        else:
            if 'file' not in request.files:
                return jsonify({"error": "Không tìm thấy file nào trong request."}), 400
