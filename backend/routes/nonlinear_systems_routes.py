...
def solve_system_route():
    try:
        ...
        if method == 'newton':
            ...
            center_str_list = data.get('center', [])
            radius_value = data.get('radius')
            if n != len(center_str_list):
                 return jsonify({"error": "Số lượng phương trình và số lượng tọa độ tâm alpha phải bằng nhau."}), 400
            
            try:
                alpha_list = [float(value) for value in center_str_list]
                radius = float(radius_value)
            except (ValueError, TypeError):
                 return jsonify({"error": "Tọa độ tâm alpha hoặc bán kính r không hợp lệ."}), 400
